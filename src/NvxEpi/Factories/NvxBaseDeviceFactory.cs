...
{
    public const string MinumumEssentialsVersion = "2.42.3";
    ...
    {
        var result = DeviceManager.GetDeviceForKey(key) as NvxXioDirector ?? throw new NullReferenceException(key);
        return result;
    }
    ...
    protected static Func<DmNvxBaseClass> GetDeviceBuildAction(string type, NvxDeviceProperties props)
    {
        try
        {
            switch (type.ToLower())
            {
                ...
                //                 : new DmNvx385((uint)props.DeviceId, xio.Hardware.Domain.Values.FirstOrDefault(), !props.DeviceIsTransmitter());
                //         };
                //     }
                //     {
                //         if (string.IsNullOrEmpty(props.ParentDeviceKey) ||
                ...
                //                 : new DmNvx385C((uint)props.DeviceId, xio.Hardware.Domain.Values.FirstOrDefault(), !props.DeviceIsTransmitter());
                //         };
                //     }
                default:
        ...
        {
            Debug.LogError("Error building NVX device of type {type}: {message}", type, ex.Message);
