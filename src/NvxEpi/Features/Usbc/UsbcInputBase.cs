...
public abstract class UsbcInputBase : INvxUsbcInput
{
    protected readonly Dictionary<uint, IntFeedback> _capability = new();
    protected readonly Dictionary<uint, BoolFeedback> _sync = new();
    protected readonly Dictionary<uint, StringFeedback> _currentResolution = new();
    protected readonly Dictionary<uint, IntFeedback> _audioChannels = new();
    protected readonly Dictionary<uint, StringFeedback> _audioFormat = new();
    protected readonly Dictionary<uint, StringFeedback> _colorSpace = new();
    protected readonly Dictionary<uint, StringFeedback> _hdrType = new();
    protected readonly Dictionary<uint, StringFeedback> _capabilityString = new();
    protected readonly Dictionary<uint, StringFeedback> _hdcpSupport = new();

    private readonly INvx38XDeviceWithHardware _device;

    protected UsbcInputBase(INvx38XDeviceWithHardware device)
    {
        _device = device;
    }
