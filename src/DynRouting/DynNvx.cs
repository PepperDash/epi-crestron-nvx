        ...
        public DynNvx(IDynNvxBuilder builder) : base(builder.Key)
        {
            AddPreActivationAction(() =>
            {
                _transmitters = builder
                    .Transmitters
                    .ToDictionary(x => x.Key, x => DeviceManager.GetDeviceForKey(x.Value) as NvxDevice);
            });
            ...
            AddPreActivationAction(() =>
            {
                ...
                {
                    Debug.Console(1, this, "Receiver at input {0} is null!", receiver.Key);
