...
namespace NvxEpi.Device.Services.FeedbackExtensions
{
    public static class CurrentAudioRouteFeedback
    {
        ...
        public static bool TryGetCurrentAudioRoute(this IHasSecondaryAudioStream device, out INvxDevice result)
        {
            try
            {
                result = null;
                ...
                if (device.Hardware.Control.ActiveAudioSourceFeedback == DmNvxControl.eAudioSource.SecondaryStreamAudio)
                {
                    result = DeviceManager
                        ...
                        .Where(t => t.IsTransmitter.BoolValue && t.IsStreamingSecondaryAudio.BoolValue)
                        .FirstOrDefault(
                            x =>
                                x.SecondaryAudioMulticastAddress.StringValue.Equals(
